...
type optionHandler func(w *TelnetConnection, data []byte) error
...
func Connect(address string, timeout time.Duration) (*TelnetConnection, error) {
	var connection TelnetConnection
	var err error

	connection.state = stateNotReady
	d := net.Dialer{Timeout: timeout}
	connection.conn, err = d.Dial("tcp", address)
	if err != nil {
		return &TelnetConnection{}, err
	}
...
func (p TelnetConnection) GetConn() net.Conn {
	return p.conn
}

// Read implements standard io.Reader interface
func (p TelnetConnection) Read(b []byte) (n int, err error) {
...
func (p TelnetConnection) Write(b []byte) (n int, err error) {
	if p.state == stateNotReady {
		return 0, errors.New("telnet connection is not ready")
	}
...
func (p *TelnetConnection) Close() error {
	p.state = stateNotReady
	return p.conn.Close()
}

// AddOption adds Telnet option to handle by Connection
func (p *TelnetConnection) addOption(optionCode OptionCode, weWill bool, peerDo bool, sbHandler optionHandler, doHandler optionHandler) {
	if p.options == nil {
...
func (p *TelnetConnection) DisableRemoteEcho() error {
...
func (p *TelnetConnection) setWindowSize(width, height int) {
	if len(p.winSize) != 4 {
...
func (p *TelnetConnection) processCommands(buffer *bytes.Buffer, outputData *bytes.Buffer) error {
...
func (p TelnetConnection) negotiateOption(data []byte) error {
...
func (p TelnetConnection) sendSB(option OptionCode, operation Operation, data []byte) error {
...
func terminalTypeOptionHandler(w *TelnetConnection, data []byte) error {
...
func negotiateAboutWindowSizeOptionHandler(w *TelnetConnection, data []byte) error {
...
func terminalSpeedOptionHandler(w *TelnetConnection, data []byte) error {
	operation := Operation(data[3])
	if operation == SEND {
